...
const JobListings = ({ jobs: propJobs, onBack }: JobListingsProps) => {
			...
			const fetchJobs = async () => {
				try {
					const response = await axios.get(
						'https://9loe6yy9pk.execute-api.us-east-1.amazonaws.com/prod/jobs'
					);
	...
	const formatDate = (dateString: string) => {
		...
		const month = date
			...
			.toUpperCase(); // 'short' gives abbreviated month
	...
	return (
		<div className="container mx-auto">
			<button
				onClick={onBack}
				className="bg-custom-black hover:bg-custom-orange text-center align-center text-white p-2 rounded mt-3 ml-3 md:ml-0 justify-center mx-autos flex flex-row"
			>
				&#5176;{' '}
				<Image
					src="/images/Jobhub Logo.png"
					alt="Job Hub"
					className="onClick={onBack} md:w-16"
					width="60"
					height="150"
				/>{' '}
			</button>
				...
				<div className="cursor-pointer grid grid-cols-1 md:grid-cols-2 gap-4 text-custom-black border mb-6 px-2 md:px-0">
					...
					{jobsToShow.map(job => (
						<Link
							href={job.link}
							key={job.id}
							className="block border-2 border-gray-300 rounded-lg p-4 hover:border-custom-orange transition w-full"
						>
							<div className="grid grid-cols-5">
								<div className="col-span-1 text-center">
									<p className="font-bold text-xl text-custom-orange">
										{formatDate(job.date).split(' ')[1]}
									</p>
									<p className="font-bold text-xl text-custom-black">
										{formatDate(job.date).split(' ')[2]}
									</p>
								</div>
				...
				<div className="flex justify-center items-center mb-2">
					{visibleJobsCount < jobs.length && (
						<button
							...
							className="bg-custom-black hover:bg-custom-orange text-center text-white p-2 rounded my-2 justify-center mx-auto"
						>
							Load More
						</button>
					)}
